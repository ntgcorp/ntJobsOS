...
def NF_StrSha256(input_string):

# Crea un oggetto hash SHA-256
    sha256_hash = hashlib.sha256()
    
# Aggiorna l'oggetto hash con la stringa codificata in bytes
    sha256_hash.update(input_string.encode('utf-8'))
    
# Restituisce l'hash come una stringa esadecimale
    return sha256_hash.hexdigest()


def NF_FileSha256(file_path):
    sProc="FILE.SHA"
    sHash=""
...
# Apri il file in modalità binaria
    lResult=nlSys.NF_FileOpen(file_path,"rb")
    sResult=lResult[1]

    if sResult=="":
        f=lResult[1]
# Leggi il file a blocchi per evitare di caricare l'intero file in memoria
        for byte_block in iter(lambda: f.read(4096), b""):
            sha256_hash.update(byte_block)
    
# Restituisci l'hash come una stringa esadecimale
        sHash=sha256_hash.hexdigest()
        
# Fine
    return [sResult,nlSys.NF_ErrorProc(sResult,sProc)]
